...
class UserReportService {
	...
	static async mostWantedPositions(industry_id, date_from, date_to) {
		...
		let roles = await models.Role.findAll({
			include: {
				...
				where: {
					...
					[operator.or]: [{ finished_at: { [operator.gte]: date_from } }, { finished_at: null }]
				},
				include: {
					model: models.User,
					include: {
		...
		roles.forEach(role => {
			...
			role.userRoles.forEach(userRole => {
				userRole.user.salaries.forEach(salary => {
					salaryArr.push(salary.salary_amount)
				})
			})
			...
			result.push({
				id: role.id,
				title: role.title,
				user_length: role.userRoles.length,
				min_sallary: Math.min(...salaryArr),
				max_sallary: Math.max(...salaryArr),
				avr_sallary: salaryArr.reduce((a, b) => a + b, 0) / salaryArr.length
			})
		})
	...
	static async mostWantedProfessions(industry_id, date_from, date_to) {
		...
		// 	profession.userProfession.forEach(userRole => {
		// 		userRole.user.salaries.forEach(salary => {
		// 			salaryArr.push(salary.salary_amount)
		// 		})
		// 	})
		...
		// 		user_length: profession.userProfession.length,
		// 		min_sallary: Math.min(...salaryArr),
		// 		max_sallary: Math.max(...salaryArr),
		// 		avr_sallary: salaryArr.reduce((a, b) => a + b, 0) / salaryArr.length
		// 	})
