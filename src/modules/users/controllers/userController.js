...
class userController {
	...
	static async initialize(req, res) {
		...
		if (user !== null) {
			return res.json(
				responseHelper.error('მსგავსი მეილით ანგარიში უკვე არსებობს')
			)
		}

		if (req.body.password !== req.body.c_password) {
			return res.json(
				responseHelper.error('პაროლები უნდა ემთხვეოდეს ერთმანეთს')
			)
		}
		...
		if (activation === true) {
			...
			return res.json(
				responseHelper.error(
					'Message was not sent, check phone and try again'
				)
			)
		}
	}
	...
	static async verify(req, res) {
		// verify sms code
		let SmsServicePromise = await activationService.verify(
			req.body.phone,
		...
		if (SmsServicePromise == false) {
			return res.json(
				responseHelper.error('SMS code is wrong or expired')
			)
		}
	...
	static async sendSms(req, res) {
		...
		let resentSmsStatus = await activationService.requestCode(
			req.body.phone
		)
	...
	static async fillData(req, res) {
		// validate sms code
		let status = await activationService.validate(
			req.body.phone,
			req.body.activationCode
		)
	...
	static async resetPassword(req, res) {
		let resetStatus = await passwordResetService.reset(
			req.body.phone,
			req.body.password,
			req.body.activationCode
		)
